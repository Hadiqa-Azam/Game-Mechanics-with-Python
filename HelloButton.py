...
while not game_over:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            x, y = event.pos #if mouse button gets down save its occurance position in x, y
    ...
    screen.blit(button_text,(button_rect[0] + button_width/2 - button_text.get_width()/2,
                             button_rect[1] + button_height/2 - button_text.get_height()/2)) # Blit specifies what to draw on screen and where to draw
